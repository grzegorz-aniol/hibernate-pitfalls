...
class SlowdownReadingMultipleEntitesTest {
    ...
    fun `problem - running query with full hibernate context loaded is less performant - this will fail`() {
        val metrics = transactionTemplate.execute {
            loadEntitiesByQuery(clearEvery = Int.MAX_VALUE, message = "Loading data without cleanup")
        }
        assertThat(metrics?.p90)
            .`as`("Queries time should be uniform")
            .isCloseTo(metrics?.avg, withPercentage(20.0))
    }
    ...
    fun `solution - clear often the context`() {
        val metrics = transactionTemplate.execute {
            loadEntitiesByQuery(clearEvery = 100, message = "Loading data with cleanup")
        }
        assertThat(metrics?.p90)
            .`as`("Queries time should be uniform")
            .isCloseTo(metrics?.avg, withPercentage(20.0))
